...
from model.unet_blocks import UNetMidBlock2D, get_down_block, get_up_block, UNetMidBlockP3D
...
@dataclass
class UNetP3DOutput(BaseOutput):
    """
    Args:
        sample (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)`):
            Hidden states output. Output of last layer of model.
...
class UNetP3DModel(ModelMixin, ConfigMixin):
    r"""
    UNet2DModel is a 2D UNet model that takes in a noisy sample and a timestep and returns sample shaped output.
    This model inherits from [`ModelMixin`]. Check the superclass documentation for the generic methods the library
    implements for all the model (such as downloading or saving, etc.)
    ...
    def __init__(
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            self.down_blocks.append(down_block)
        # mid
        self.mid_block = UNetMidBlockP3D(
            in_channels=block_out_channels[-1],
            temb_channels=time_embed_dim,
            resnet_eps=norm_eps,
            resnet_act_fn=act_fn,
        ...
        self.conv_norm_out = nn.GroupNorm(num_channels=block_out_channels[0],
                                          ...
                                          eps=norm_eps)
        self.conv_act = nn.SiLU()
        self.conv_out = nn.Conv3d(block_out_channels[0],
                                  out_channels, (1, 3, 3),
                                  padding=(0, 1, 1))
    ...
    def forward(
        ...
        return UNetP3DOutput(sample=sample)


if __name__ == "__main__":
    model = UNetP3DModel.from_config('./models/UnetP3D/config.json')
    inputs = torch.randn(4, 3, 16, 64, 64)
    outputs = model(inputs, timestep=100)
    import pdb
    pdb.set_trace()
    pass
