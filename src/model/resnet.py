...
from model.layer import Pseudo3DConv, SpatioTemporalAttention
...
class Upsample3D(nn.Module):
    ...
    def __init__(self,
        ...
        conv = None
        if use_conv_transpose:
            conv = nn.ConvTranspose3d(channels, self.out_channels, (1, 4, 4),
                                      (1, 2, 2), (0, 1, 1))
        elif use_conv:
            conv = nn.Conv3d(self.channels,
                             self.out_channels, (1, 3, 3),
                             padding=(0, 1, 1))
...
class Downsample3D(nn.Module):
    ...
    def __init__(self,
        ...
        if use_conv:
            ...
            assert self.channels == self.out_channels
            conv = nn.AvgPool3d(kernel_size=(1, stride, stride),
                                stride=(1, stride, stride))
    ...
    def _upsample_2d(self,
        ...
        if self.use_conv:
            ...
            weight = torch.reshape(weight,
                                   (num_groups * inC, -1, convH, convW))

            inverse_conv = F.conv_transpose2d(hidden_states,
                                              weight,
                                              stride=stride,
        ...
        *,
        ...
        if self.up:
            ...
            if kernel == "fir":
                ...
                self.downsample = partial(F.avg_pool2d,
                                          kernel_size=2,
                                          stride=2)
            else:
...
class ResnetBlockP3D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.conv1 = Pseudo3DConv(dim=in_channels,
        ...
        if temb_channels is not None:
            self.time_emb_proj = torch.nn.Linear(temb_channels, out_channels)
            self.fps_emb_proj = torch.nn.Linear(temb_channels, out_channels)
        else:
            self.time_emb_proj = None
            self.fps_emb_proj = None
        ...
        self.norm2 = torch.nn.GroupNorm(num_groups=groups_out,
                                        ...
                                        affine=True)
        self.dropout = torch.nn.Dropout(dropout)
        self.conv2 = Pseudo3DConv(dim=out_channels,
                                  dim_out=out_channels,
                                  kernel_size=3,
                                  stride=1)
    ...
    def forward(self, input_tensor, temb, femb=None):
        ...
        if temb is not None:
            ...
            hidden_states = hidden_states + temb
        if femb is not None:
            femb = self.fps_emb_proj(self.nonlinearity(femb))[:, :, None, None,
                                                              None]
...
if __name__ == "__main__":
    ...
    images = torch.randn(1, 128, 16, 16)  # (batch, features, height, width)
    import pdb
    pdb.set_trace()
