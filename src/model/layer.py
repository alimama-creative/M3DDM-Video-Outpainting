...
class Attention(nn.Module):
    ...
    def __init__(self, dim, dim_head=64, heads=8):
        ...
        nn.init.zeros_(
            self.to_out.weight.data)  # identity with skip connection
...
class Pseudo3DConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_conv = nn.Conv2d(dim,
                                      dim_out,
                                      kernel_size=kernel_size,
                                      padding=kernel_size // 2)
        self.temporal_conv = nn.Conv1d(dim_out,
                                       dim_out,
                                       kernel_size=temporal_kernel_size,
                                       padding=temporal_kernel_size // 2)

        nn.init.dirac_(
            self.temporal_conv.weight.data)  # initialized to be identity
...
class SpatioTemporalAttention(nn.Module):
    ...
    def forward(self, x, attend_across_time=True):
        b, c, *_, h, w = x.shape
        is_video = x.ndim == 5
...
if __name__ == "__main__":
    ...
    conv = Pseudo3DConv(dim=256, kernel_size=3)
